"""SOT-1507: 新規ユーザー初回ログイン時の初期データ配布（案B）。

現在の既定オーナー（sota.moro@gmail.com）のデータを「初期データの正」とし、新規ユーザーが
初めてログインした時に、そのデータ（やることタスク ``NurseryInfo`` と子ども ``Child``）を当該
ユーザーの ``owner_id`` へ独立したコピーとして配布する。コピー後は各ユーザーが自由に編集できる。

- オーナーごとに一度だけ実行する（冪等）。``SeededOwner`` マーカーで担保する。
...
def ensure_user_seeded(owner_id: str) -> bool:
    """``owner_id`` が初回なら既定オーナーの初期データをコピーする。コピーしたら ``True``。

    既定オーナー自身・既にシード済みのオーナー・空の owner_id はスキップして ``False`` を返す。
    ベストエフォートのため、いかなる失敗でも例外を送出しない。
    """
