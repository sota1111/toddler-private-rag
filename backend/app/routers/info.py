...
def create_info(info: schemas.NurseryInfoCreate, db: Session = Depends(get_db)):
...
def get_tomorrow_info(db: Session = Depends(get_db)):
...
def get_weekly_info(db: Session = Depends(get_db)):
    today = date.today()
...
def get_pending_info(db: Session = Depends(get_db)):
    return db.query(models.NurseryInfo).filter(
...
def update_info(id: int, info: schemas.NurseryInfoUpdate, db: Session = Depends(get_db)):
    db_info = db.query(models.NurseryInfo).filter(models.NurseryInfo.id == id).first()
    if db_info is None:
        raise HTTPException(status_code=404, detail="Info not found")
