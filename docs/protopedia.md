...
- **エージェント性能評価ゲート**（`evaluation-gate`, `backend/tests/test_eval_ocr.py` / `test_eval_rag.py`,
  golden dataset は `backend/tests/eval/dataset.py`）を **独立した必須ステータスチェック**として分離。
  閾値を割ると CI が落ち、**CD（本番デプロイ）もブロック**されます。
  - **OCR** — 日付・持ち物の coverage（recall）に加え **precision（誤検出ゼロ）** と **F1** を下限ゲート。
  - **RAG** — **top-source 正答率** / **keyword hit rate** / **groundedness**（回答語が取得ソースに追跡可能か）/
    **refusal**（空インデックス時は出典なしで拒否）を下限ゲート。
