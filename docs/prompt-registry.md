...
| 1.1.0 | 2026-08-20 | SOT-2736: `rag_answer` に「安全に関する制約」ブロックを追加。健康・安全の断定禁止（安全側の断定も禁止）、責任境界（最終判断=保護者/専門判断=医療者・園）、相反情報の両論提示を指示。出力側は `app/safety_guard.apply_output_guard` で二重に中和する。 |
