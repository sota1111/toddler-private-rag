...
エージェントの自律調査（Google Search grounding）や所要期間の見積りは、**必ずしも正しいとは限らず、
必ずしも利用者の意図通りとも限りません**。発行元や標準的な所要期間の推定を誤れば、自動生成された準備
タスクの分割や日付もずれ得ますし、たとえ調査結果そのものが誤りでなくても、分割の粒度や生成された内容が
利用者の想定と食い違うこと（＝**意図通りでない**）もあります。ここで、間違った・意図と異なる結果を
**取り消せない**と、利用者はエージェントを信頼できず、かえって不満を抱えて使わなくなってしまいます。
...
#### 数値で見る品質 — eval-gate の閾値と現在値

「測っている」だけでなく **数字を見せます**。以下はすべて実コードの閾値（`MIN_*` 定数）と、golden
dataset に対する eval スイートの実測値です。閾値を割ると `evaluation-gate` が失敗し、**本番デプロイも
ブロック**されます。

| 指標（eval-gate） | ゲート閾値（下限） | 現在値 | 守る品質 |
|---|---|---|---|
| OCR 日付 — coverage / precision / F1 | ≥ 0.80 | 1.00 / 1.00 / 1.00 | 締切・行事日の抽出漏れ／誤検出を防ぐ |
| OCR 持ち物 — coverage / precision / F1 | ≥ 0.80 | 1.00 / 0.97 / 0.98 | 持ち物の抽出漏れ／誤検出を防ぐ |
| RAG top-source 正答率 | ≥ 0.80 | 1.00 | 回答の根拠を最上位ソースに引けているか |
| RAG keyword hit rate | ≥ 0.80 | 1.00 | 回答に必要な語の取りこぼしを防ぐ |
| RAG groundedness | ≥ 0.80 | 1.00 | ソースに無い語＝幻覚を答えに混ぜない |
| RAG refusal（空インデックス時の拒否） | pass/fail | pass | 検索ゼロ時に回答を捏造せず拒否 |
| エージェント タスク生成 — coverage / precision | ≥ 0.80 | 1.00 / 1.00 | 期待「やること」を漏れなく・過検出なく生成 |
| エージェント 締切逆算 一致率（許容誤差 0 日） | ≥ 0.80 | 1.00 | 逆算日付が golden と完全一致（README 実例 7/30→7/9 を含む） |
...
| **3. ユーザビリティ** | 撮るだけ登録 / 今日やること集約 / 出典付き回答 / モバイルファースト（特徴4）。自律調査が誤っても・意図と食い違っても**分割前に戻す・再逆算・編集**で人間が主導権を取り戻せる（特徴1） |
