...
// SOT-1510: アプリ内ブラウザ（埋め込み WebView）では Google が OAuth をポリシーで拒否する
// （Error 403: disallowed_useragent）。ブロック画面へ飛ばす前に検出し、標準ブラウザで開くよう
// 案内して外部遷移を発火させないこと。
test.describe('アプリ内ブラウザからの Google ログイン (SOT-1510)', () => {
  test.use({
    userAgent:
      'Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 [FBAN/FBIOS;FBAV/400.0.0.0.0]',
  })

  test('Googleログインを押すと標準ブラウザ利用を案内し /login に留まる', async ({ page }) => {
    await mockApi(page, { authed: false })
    await page.goto('/login')
    await page.getByRole('button', { name: 'Googleでログイン' }).click()
    // 案内メッセージが表示され、外部（accounts.google.com）へ遷移せず /login に留まる。
